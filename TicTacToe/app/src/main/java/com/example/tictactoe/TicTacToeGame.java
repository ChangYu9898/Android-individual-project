...
public class TicTacToeGame {
    ...
    public int getComputerMove(int Android_Diff,boolean Android_Move_First)
    {
        int move;
        int[] mBoard_corner = new int[]{0, 2, 6, 8};
        AndroidMove = AndroidMove+1;
        if(Android_Diff !=0){
            //The best way to win is to take the corner of the board
            if(Android_Diff == 2&&Android_Move_First==true){
                move = mBoard_corner[mRand.nextInt(4)];
            ...
            if (Android_Diff == 2) {
                if (AndroidMove == 3 && (mBoard[4] == COMPUTER_PLAYER)) {
                    for (int i : mBoard_corner) {
                        if (mBoard[i] != HUMAN_PLAYER && mBoard[i] != COMPUTER_PLAYER) {
                            mBoard[i] = COMPUTER_PLAYER;
                            if (win()) {
                                return i;
                ...
                if (AndroidMove == 2 && (mBoard[1] == HUMAN_PLAYER || mBoard[3] == HUMAN_PLAYER || mBoard[5] == HUMAN_PLAYER || mBoard[7] == HUMAN_PLAYER)) {
                    move = 4;
                    mBoard[move] = COMPUTER_PLAYER;
                    return move;
                }
